...
exit 1 unless @ARGV == $NUMBER_OF_ARGS;               ## T parameters

my( $repos, $rev, $auth_user, $prop_name, $description ) = @ARGV;             ## Repository name and revision
...
##use Data::Dumper; open FH,'>',"/tmp/svn-$$"; print FH Dumper( $config->info('mirrors') ); close FH;
  $support->read_from_process( qw(/usr/bin/svnsync copy-revprops --non-interactive --sync-username),
    $mirror_config->{'user'},
