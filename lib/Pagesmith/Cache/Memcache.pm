...
use Pagesmith::Cache::Base qw(expires columns text_mode);
...
sub get {    ## Returns value of undef if not found!
  ...
  utf8::upgrade( $content ) if text_mode( $key ); ## no critic (CallsToUnexportedSubs)
...
sub set {    ## Returns true if set was successful...
  ...
  utf8::downgrade( $content ) if text_mode( $key ); ## no critic (CallsToUnexportedSubs)
