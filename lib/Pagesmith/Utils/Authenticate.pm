package Pagesmith::Utils::Authenticate;

## Ask an authentication server whether user/password is ok
## Author         : mw6
## Maintainer     : mw6
## Created        : 2009-08-12
...
sub create_path {
  ...
  my $checksum = safe_md5( join q(:), $self->{'secret'}, $encrypted_user, $encrypted_password );
...
sub lwp_get {
  ...
  my $res;
  my $rv = eval {
    my $req = HTTP::Request->new('POST' => "$self->{'server'}/action/authenticate/$path" );
       $req->content_type('application/x-www-form-urlencoded');
       $req->content(q());
    my $ua = LWP::UserAgent->new;
       $ua->agent('Pagesmith_User_Auth/0.1');
...
sub check_authentication {
  ...
  return 'Unknown user' unless $path;

  my $res  = $self->lwp_get( $path );

  return 'System failure' unless $res && $res->is_success;

  return 'Unknown user'   unless $res->content;

  my $json_obj;
  my $rv = eval { $json_obj = $self->json_decode( $self->cipher->decrypt( safe_base64_decode( $res->content ) ) ); };

  return 'Unknown user' unless $json_obj;
...
Usage: To authenticate against this service you will need, and $API_KEY and related $key and $secret
