...
var cs_defaults = {
  ...
  xaxis:      { label_style: '', minvalue: null, maxvalue: null, dir: '+', label: '', line: null, scaling: 'linear', major: 1, minor: 1, axis_pos: 'below', labelsize: 14, size: 10, ticksize: 4, values: [], label_scale: 0, label_dp: 0, label_suffix: '' }, // Configurat
  yaxis:      { label_style: '', minvalue: null, maxvalue: null, dir: '-', label: '', line: null, scaling: 'linear', major: 1, minor: 1, axis_pos: 'left',  labelsize: 14, size: 10, ticksize: 4, values: [], label_scale: 0, label_dp: 0, label_suffix: '' },
  background: '#eee',
...
Raphael.fn.cs_init = function (pars) {
  ...
  Chartsmith_Collector[pars.object] = this;
  this.cs = cs_defaults;
  this.notIE = document.all ? false : true;
  this.cs_merge([ 'xaxis', 'yaxis', 'background', 'title', 'edge' ], pars);
// Default margin = 20px
...
//   * the x-axis numbering is on the bottom edge!
  if (this.cs.xaxis.axis_pos === 'below' || this.cs.xaxis.scaling  === 'discrete' || (this.cs.xaxis.axis_pos === 'on' && (this.cs.xaxis.scaling === 'log' ? 1 : 0) < this.cs.xaxis.minvalue)) {
...
Raphael.fn.cs_draw_canvas = function () {
  var o = this, x_ticks, y_ticks, i, tck, t, x, v, mu, x2, v2, mnu, o2, val, B, bw, tb1, BB1, BB2, tb2, BB3, tb3, mb, A, ma, AA;
/*
Configure the axes
*/
...
/** Draw x-axis **/
  ...
  for (i = x_ticks.length; i; i--) {
    ...
    this.cs_add_line({
      raw: 1,
      stroke: '#000',
      pts: [ tck.pos, this.cs.xaxis.pos, tck.pos, this.cs.xaxis.pos + this.cs.xaxis.ticksize ]
    });
    if (typeof (tck.label) !== 'undefined') {
      ...
      if (mb) {
        ...
        BB1 = 0;
        if (mb[3]) {
          tb1 = this.cs_add_text({ raw: 1, x: tck.pos, y: this.cs.xaxis.pos + this.cs.xaxis.ticksize + this.cs.xaxis.size / 2 + 8, t: mb[2], opts: { fill: '#000', 'font-size': this.cs.xaxis.size + 'pt' } });
          BB1 = tb1.getBBox().width / 2;
        }
        tb2 = this.cs_add_text({ raw: 1, x: tck.pos, y: this.cs.xaxis.pos + this.cs.xaxis.ticksize + this.cs.xaxis.size / 2 + 8 - 3, t: mb[1], opts: { fill: '#000', 'font-size': this.cs.xaxis.size * 0.6 + 'pt' } });
        BB2 = tb2.getBBox().width / 2;
        tb3 = this.cs_add_text({ raw: 1, x: tck.pos, y: this.cs.xaxis.pos + this.cs.xaxis.ticksize + this.cs.xaxis.size / 2 + 8, t: '10', opts: { fill: '#000', 'font-size': this.cs.xaxis.size + 'pt' } });
        ...
        this.cs_add_text({ raw: 1, x: tck.pos, y: this.cs.xaxis.pos + this.cs.xaxis.ticksize + this.cs.xaxis.size / 2 + 8, t: tck.label, opts: { fill: '#000', 'font-size': this.cs.xaxis.size + 'pt' } });
      }
    }
  }
...
Raphael.fn.cs_add_points = function (pts, defs) {
  var t = this.set(), i, defaults = {radius: 5, edge: undefined, opacity: 0.5, col: '#000', shape: 'circle', show_label: false, popup: true, 'stroke-width': 1 }, defo, pt, x, y, c, title, txl, pr, w, h, xo, yo, xalign, yalign, xpos, ypos, o = this, mout, mover;
  ...
  for (i = pts.length; i; i--) {
    ...
    c.attrs.cy = y;
    c.attrs.r  = pt.radius;
    title = pt.value + " \n(" + this.cs_format_x(pt.x) + ',' + this.cs_format_y(pt.y) + ')';
    if (pt.label) {
