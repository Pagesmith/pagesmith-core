...
(function($){
  ...
  /* Generating results methods */
    format_doc: function ( doc, hl ) {
      ...
      if( this.tmpls[facet] ) {
        ...
        body_txt = '<dl class="twocol">'+$.map(defn.fields,function(v,k) { return '<dt>'+v+'</dt><dd>'+(hl[k]?hl[k][0]:Pagesmith.escapeHTML(doc[k]))+'</dd>'; } ).join('')+'</dl>';
        /*jsl:end*/
        facet = defn.label;
      } else {
        /*jsl:ignore*/
        url       = Pagesmith.escapeHTML(doc.domain_url);
        /*jsl:end*/
    ...
    render_results: function ( data ) {
      ...
      /*jsl:ignore*/
      var that=this, header = '<h2>Search results for <em>'+Pagesmith.escapeHTML(data.responseHeader.params.q==='*:*'?'*':data.responseHeader.params.q)+'</em>';
      /*jsl:end*/
      if( this.facet ) {
  ...
  /* Handle an individual search */
    search: function () {
      ...
      /*jsl:ignore*/
      $('#search_waiting span').html( 'for <em>'+Pagesmith.escapeHTML(this.par.q)+'</em>'+
      /*jsl:end*/
